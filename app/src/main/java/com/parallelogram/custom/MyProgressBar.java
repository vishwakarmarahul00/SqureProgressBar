...
public class MyProgressBar extends LinearLayout {
    ...
    List<MyCustomeView> barArray = new ArrayList<MyCustomeView>();
    ...
    private void intializeBarArray() {
        ...
        while(i < noOfSteps){
            ...
            MyCustomeView parralogramView=new MyCustomeView(context,xvalue,0,barWidth,barHeight,waitingColor);
    ...
    public void updateProgress(int step){
        if (step >= -1 && step < noOfSteps){
            int i = 0;
            while(i < noOfSteps){
                MyCustomeView k = barArray.get(i);
                if(i <= step){
                    if(!k.isMarkComplete()){
                        MyCustomeView toggeledBar = new MyCustomeView(context,0,0,k.getBarWidth(),k.getBarHeight(),completeColor);
                    ...
                    if(k.isMarkComplete()){
                        MyCustomeView toggeledBar = new MyCustomeView(context,0,0,k.getBarWidth(),k.getHeight(),waitingColor);
